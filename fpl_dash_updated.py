...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Tabs(id='main-tabs', value='home', children=[
            ...
            # BGW / DGW PLANNER TAB
            # =================================================================
            dcc.Tab(label='BGW / DGW Planner', value='bgw-planner', children=[
                html.Div([

                    html.Div([
                        html.H3("Blank & Double Gameweek Planner",
                                style={'color': COLORS['primary'], 'marginBottom': '12px'}),
                        html.P([
                            "Plan your transfers and chip usage weeks ahead. ",
                            html.Strong("Green = Double Gameweek (2 fixtures)"), ", ",
                            html.Strong("Grey = Single Gameweek"), ", ",
                            html.Strong("Red = Blank Gameweek (no fixture)"), ". ",
                            "Target players from teams with DGWs and avoid blanking premium assets."
                        ], style={'color': COLORS['text_dark'], 'fontSize': '15px', 'marginBottom': '12px'}),
                        html.Div([
                            html.Span("All remaining gameweeks from current GW onwards",
                                      style={'backgroundColor': COLORS['secondary'], 'color': COLORS['primary'],
                                             'padding': '8px 16px', 'borderRadius': '20px', 'fontWeight': '600'})
                        ])
                    ], style={**CARD_STYLE, 'backgroundColor': '#f8f9fa'}),
                    ...
                    # Heatmap
                    html.Div([
                        html.H3("Fixture Schedule Heatmap",
                                style={'color': COLORS['primary'], 'marginBottom': '8px'}),
                        html.P("Each cell shows the opponent(s). DGW = two fixtures that week.",
                               style={'color': COLORS['text_light']}),
                        dcc.Graph(id='bgw-heatmap')
                    ], style=CARD_STYLE),

                    # Filters for player table
                    html.Div([
                        html.Div([
                            html.Div([
                                html.Label("Position",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Dropdown(id='bgw-position',
                    ...
                    # Player table
                    html.Div([
                        html.H4("Player Fixture Summary (Remaining Season)",
                                style={'color': COLORS['primary'], 'marginBottom': '16px'}),
                        html.P("DGWs = number of double gameweeks remaining. Blanks = number of blank gameweeks.",
                        ...
                        dash_table.DataTable(
                            ...
                            columns=[
                                ...
                                {'name': 'PPG', 'id': 'ppg', 'type': 'numeric',
                                 'format': {'specifier': '.2f'}},
                                {'name': 'Form', 'id': 'form', 'type': 'numeric',
                                 'format': {'specifier': '.1f'}},
                                {'name': 'DGWs Left', 'id': 'dgw_count', 'type': 'numeric'},
                                {'name': 'Blanks Left', 'id': 'blank_count', 'type': 'numeric'},
                                {'name': 'Next 5 Fixtures', 'id': 'fixture_string'},
                                {'name': 'Own%', 'id': 'ownership', 'type': 'numeric',
                                 'format': {'specifier': '.1f'}},
                            ],
                            sort_action='native',
                            page_size=20,
                            ...
                            style_data_conditional=[
                                {'if': {'row_index': 'odd'}, 'backgroundColor': '#fafafa'},
                                {'if': {'filter_query': '{dgw_count} >= 2', 'column_id': 'dgw_count'},
                                 'backgroundColor': '#e8f5e9', 'fontWeight': '600'},
                                {'if': {'filter_query': '{dgw_count} = 1', 'column_id': 'dgw_count'},
                                 'backgroundColor': '#f1f8e9'},
                                {'if': {'filter_query': '{blank_count} >= 2', 'column_id': 'blank_count'},
                                 'backgroundColor': '#ffebee'},
                            ]
                        )
                    ], style=CARD_STYLE),
            ...
            # =================================================================
            # UNDERLYING STATS PER 90 TAB
            # =================================================================
            dcc.Tab(label='Underlying Stats / 90', value='u90', children=[
                html.Div([

                    html.Div([
                        html.H3("Underlying Stats Per 90 Minutes",
                                style={'color': COLORS['primary'], 'marginBottom': '12px'}),
                        html.P([
                            "Total season stats flatter players with more minutes and punish those who are rotated. ",
                            "Per-90 metrics normalise for playing time, surfacing players who are ",
                            html.Strong("consistently generating chances"), " regardless of whether they've converted. ",
                            "These are the players whose returns are most likely to be sustainable."
                        ], style={'color': COLORS['text_dark'], 'fontSize': '15px', 'marginBottom': '12px'}),
                        html.Div([
                            html.Span("xG/90 and xA/90 predict future returns better than goals and assists alone",
                                      style={'backgroundColor': COLORS['secondary'], 'color': COLORS['primary'],
                                             'padding': '8px 16px', 'borderRadius': '20px', 'fontWeight': '600'})
                        ])
                    ], style={**CARD_STYLE, 'backgroundColor': '#f8f9fa'}),
                    ...
                    html.Div([
                        html.Div([
                            html.Div([
                                html.Label("Position",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Dropdown(id='u90-position',
                                             options=[{'label': 'All', 'value': 'All'}] +
                                                     [{'label': p, 'value': p} for p in ['GKP', 'DEF', 'MID', 'FWD']],
                                             value='MID', clearable=False)
                            ], style={'flex': '1', 'minWidth': '150px', 'padding': '0 10px'}),
                            html.Div([
                                html.Label("Team",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Dropdown(id='u90-team',
                                             options=[{'label': 'All', 'value': 'All'}] +
                                                     [{'label': t, 'value': t} for t in sorted_teams],
                                             value='All', clearable=False)
                            ], style={'flex': '1', 'minWidth': '150px', 'padding': '0 10px'}),
                            html.Div([
                                html.Label("Max. price (£m)",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Slider(id='u90-price', min=4, max=16, step=0.5, value=16,
                                           marks={i: f'£{i}' for i in [4, 6, 8, 10, 12, 14, 16]},
                                           tooltip={"placement": "bottom", "always_visible": True})
                            ], style={'flex': '2', 'minWidth': '200px', 'padding': '0 10px'}),
                            html.Div([
                                html.Label("Min. minutes",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Input(id='u90-minutes', type='number', value=450, min=0, step=50,
                                          style={'width': '100%', 'padding': '8px', 'borderRadius': '4px',
                                                 'border': '1px solid #ccc'})
                            ], style={'flex': '1', 'minWidth': '100px', 'padding': '0 10px'}),
                            html.Div([
                                html.Label("Bar Chart Metric",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Dropdown(id='u90-metric',
                                             options=[
                                                 {'label': 'xGI per 90', 'value': 'xgi_per_90'},
                                                 {'label': 'xG per 90', 'value': 'xg_per_90'},
                                                 {'label': 'xA per 90', 'value': 'xa_per_90'},
                                                 {'label': 'Goals per 90', 'value': 'goals_per_90'},
                                                 {'label': 'Assists per 90', 'value': 'assists_per_90'},
                                                 {'label': 'BPS per 90', 'value': 'bps_per_90'},
                                             ],
                                             value='xgi_per_90', clearable=False)
                            ], style={'flex': '2', 'minWidth': '180px', 'padding': '0 10px'}),
                        ], style={'display': 'flex', 'flexWrap': 'wrap', 'alignItems': 'flex-end'})
                    ], style=CARD_STYLE),

                    # Scatter: xG/90 vs xA/90
                    html.Div([
                        html.H3("xG per 90 vs xA per 90",
                                style={'color': COLORS['primary'], 'marginBottom': '8px'}),
                        html.P([
                            "Top right = players generating both goals and assists at a high rate. ",
                            "Top left = creative players. Bottom right = clinical finishers. ",
                            html.Strong("Bubble size = minutes played.")
                        ], style={'color': COLORS['text_light']}),
                        dcc.Graph(id='u90-scatter')
                    ], style=CARD_STYLE),

                    # Bar: top players by selected metric
                    html.Div([
                        html.H3("Top 25 Players by Selected Metric",
                                style={'color': COLORS['primary'], 'marginBottom': '8px'}),
                        html.P("Players with high per-90 output but low ownership are the most valuable targets.",
                               style={'color': COLORS['text_light']}),
                        dcc.Graph(id='u90-bar')
                    ], style=CARD_STYLE),

                    # Table
                    html.Div([
                        html.H4("Full Per-90 Rankings",
                                style={'color': COLORS['primary'], 'marginBottom': '16px'}),
                        dash_table.DataTable(
                            id='u90-table',
                            data=[],
                            columns=[
                                ...
                                {'name': 'Mins', 'id': 'minutes', 'type': 'numeric',
                                 'format': {'specifier': ','}},
                                {'name': 'xG/90', 'id': 'xg_per_90', 'type': 'numeric',
                                 'format': {'specifier': '.2f'}},
                                {'name': 'xA/90', 'id': 'xa_per_90', 'type': 'numeric',
                                 'format': {'specifier': '.2f'}},
                                {'name': 'xGI/90', 'id': 'xgi_per_90', 'type': 'numeric',
                                 'format': {'specifier': '.2f'}},
                                {'name': 'Goals/90', 'id': 'goals_per_90', 'type': 'numeric',
                                 'format': {'specifier': '.2f'}},
                                {'name': 'Assists/90', 'id': 'assists_per_90', 'type': 'numeric',
                                 'format': {'specifier': '.2f'}},
                                {'name': 'BPS/90', 'id': 'bps_per_90', 'type': 'numeric',
                                 'format': {'specifier': '.1f'}},
                                {'name': 'Own%', 'id': 'ownership', 'type': 'numeric',
                            ...
                            style_data_conditional=[
                                {'if': {'row_index': 'odd'}, 'backgroundColor': '#fafafa'},
                                {'if': {'filter_query': '{xgi_per_90} >= 0.5', 'column_id': 'xgi_per_90'},
                                 'backgroundColor': '#e8f5e9'},
                                {'if': {'filter_query': '{xgi_per_90} >= 0.3 && {xgi_per_90} < 0.5',
                                        'column_id': 'xgi_per_90'}, 'backgroundColor': '#f1f8e9'},
                            ]
                        )
                    ], style=CARD_STYLE),
...
# --- BGW / DGW PLANNER ---
@callback(
    [Output('bgw-heatmap', 'figure'), Output('bgw-player-table', 'data')],
    [Input('bgw-position', 'value'), Input('bgw-team', 'value')]
)
def update_bgw(position, team):
    data = get_data()
    fixtures = data['fixtures_data']
    current_gw_now = data.get('current_gw')
    current_gw_num = current_gw_now['id'] if current_gw_now else 1
    teams_df = data['teams_df']
    df_now = data['df_active']

    team_id_to_name = dict(zip(teams_df['id'], teams_df['name']))
    team_names = sorted(teams_df['name'].tolist())

    # All remaining GWs (after current)
    remaining_gws = sorted(set(
        f['event'] for f in fixtures
        if f.get('event') and f['event'] > current_gw_num
    ))

    if not remaining_gws:
        empty_fig = go.Figure()
        empty_fig.add_annotation(text="No remaining fixtures found.", xref="paper", yref="paper",
                                 x=0.5, y=0.5, showarrow=False, font=dict(size=14))
        empty_fig.update_layout(template='plotly_white', height=400)
        return empty_fig, []

    # Count fixtures and build labels per team per GW
    fixture_counts = {t: {gw: 0 for gw in remaining_gws} for t in team_names}
    fixture_labels = {t: {gw: '' for gw in remaining_gws} for t in team_names}

    for f in fixtures:
        gw = f.get('event')
        if not gw or gw <= current_gw_num:
            continue
        h_name = team_id_to_name.get(f['team_h'], '')
        a_name = team_id_to_name.get(f['team_a'], '')
        if h_name and h_name in fixture_counts:
            fixture_counts[h_name][gw] += 1
            sep = ', ' if fixture_labels[h_name][gw] else ''
            fixture_labels[h_name][gw] += sep + f"{a_name} (H)"
        if a_name and a_name in fixture_counts:
            fixture_counts[a_name][gw] += 1
            sep = ', ' if fixture_labels[a_name][gw] else ''
            fixture_labels[a_name][gw] += sep + f"{h_name} (A)"

    # Sort teams: fewest blanks first, then alphabetically
    blank_counts = {t: sum(1 for gw in remaining_gws if fixture_counts[t][gw] == 0) for t in team_names}
    sorted_teams_heatmap = sorted(team_names, key=lambda t: (blank_counts[t], t))

    # Build heatmap matrix
    z_data, hover_text, cell_text = [], [], []
    for t in sorted_teams_heatmap:
        z_row, h_row, c_row = [], [], []
        for gw in remaining_gws:
            count = fixture_counts[t][gw]
            label = fixture_labels[t][gw]
            z_row.append(count)
            h_row.append(f"GW{gw} — {label if label else 'BLANK'}")
            if count == 0:
                c_row.append('–')
            elif count >= 2:
                c_row.append('DGW')
            else:
                # Shorten label: "Chelsea (H)" → "CHE (H)"
                c_row.append(label[:12] if label else '')
        z_data.append(z_row)
        hover_text.append(h_row)
        cell_text.append(c_row)

    colorscale = [
        [0.0, '#ffcdd2'],   # 0 fixtures — red
        [0.5, '#f5f5f5'],   # 1 fixture  — grey
        [1.0, '#c8e6c9'],   # 2 fixtures — green
    ]

    heatmap_fig = go.Figure(go.Heatmap(
        z=z_data,
        x=[f"GW{gw}" for gw in remaining_gws],
        y=sorted_teams_heatmap,
        text=cell_text,
    ...
    heatmap_fig.update_layout(
        template='plotly_white',
        height=max(500, len(team_names) * 28),
        xaxis=dict(side='top', tickfont=dict(size=10)),
        yaxis=dict(tickfont=dict(size=11)),
        margin=dict(l=130, r=20, t=60, b=20),
        font=dict(family='Arial, sans-serif', size=9),
    )

    # Player table
    filtered_players = df_now.copy()
    if position != 'All':
        filtered_players = filtered_players[filtered_players['position'] == position]
    if team != 'All':
        filtered_players = filtered_players[filtered_players['team_name'] == team]

    filtered_players = filtered_players[filtered_players['minutes'] >= 200].copy()
    filtered_players['dgw_count'] = filtered_players['team_name'].map(
        lambda t: sum(1 for gw in remaining_gws if fixture_counts.get(t, {}).get(gw, 0) >= 2)
    )
    filtered_players['blank_count'] = filtered_players['team_name'].map(
        lambda t: sum(1 for gw in remaining_gws if fixture_counts.get(t, {}).get(gw, 0) == 0)
    )

    cols = ['web_name', 'team_name', 'position', 'price', 'ppg', 'form',
            'dgw_count', 'blank_count', 'fixture_string', 'ownership']
    table_data = prepare_table_data(
        filtered_players.nlargest(80, 'ppg'),
        [c for c in cols if c in filtered_players.columns]
    )
    return heatmap_fig, table_data


# --- UNDERLYING STATS PER 90 ---
@callback(
    [Output('u90-scatter', 'figure'), Output('u90-bar', 'figure'), Output('u90-table', 'data')],
    [Input('u90-position', 'value'), Input('u90-team', 'value'), Input('u90-price', 'value'),
     Input('u90-minutes', 'value'), Input('u90-metric', 'value')]
)
def update_u90(position, team, max_price, min_minutes, metric):
    filtered = filter_data(position, team, max_price, min_minutes)
    filtered = filtered.copy()

    # Compute per-90 metrics
    filtered['xg_per_90'] = (filtered['expected_goals'] / filtered['minutes_safe'] * 90).round(3)
    filtered['xa_per_90'] = (filtered['expected_assists'] / filtered['minutes_safe'] * 90).round(3)
    filtered['xgi_per_90'] = (filtered['expected_goal_involvements'] / filtered['minutes_safe'] * 90).round(3)
    filtered['goals_per_90'] = (filtered['goals_scored'] / filtered['minutes_safe'] * 90).round(3)
    filtered['assists_per_90'] = (filtered['assists'] / filtered['minutes_safe'] * 90).round(3)

    filtered = filtered.dropna(subset=['xg_per_90', 'xa_per_90', 'xgi_per_90'])

    if len(filtered) == 0:
        empty_fig = go.Figure()
        empty_fig.add_annotation(text="No players match current filters.",
    ...
    # Scatter: xG/90 vs xA/90
    plot_df = filtered.copy()
    plot_df['bubble_size'] = (plot_df['minutes'] / plot_df['minutes'].max() * 30).clip(lower=3)
    scatter_fig = px.scatter(
        plot_df, x='xg_per_90', y='xa_per_90',
        color='position', size='bubble_size',
        hover_name='web_name',
        hover_data={
            'team_name': True, 'price': True,
            'xgi_per_90': ':.3f', 'goals_per_90': ':.3f',
            'assists_per_90': ':.3f', 'bubble_size': False
        },
        color_discrete_map={'GKP': '#666', 'DEF': COLORS['primary'],
                            'MID': COLORS['accent'], 'FWD': COLORS['info']}
    )
    scatter_fig.update_layout(
        template='plotly_white', height=420,
        xaxis_title='xG per 90', yaxis_title='xA per 90',
        font=dict(family='Arial, sans-serif')
    )

    # Bar: top 25 by selected metric
    metric_labels = {
        ...
        'bps_per_90': 'BPS per 90',
    }
    metric_label = metric_labels.get(metric, metric)

    pos_color_map = {'GKP': '#666', 'DEF': COLORS['primary'], 'MID': COLORS['accent'], 'FWD': COLORS['info']}
    top_25 = filtered.dropna(subset=[metric]).nlargest(25, metric)
    bar_fig = go.Figure()
    bar_fig.add_trace(go.Bar(
        x=top_25['web_name'],
        y=top_25[metric],
        marker_color=[pos_color_map.get(p, '#999') for p in top_25['position']],
        text=top_25[metric].round(3),
        textposition='outside',
        hovertemplate=(
            '%{x}<br>'
            + metric_label + ': %{y:.3f}<br>'
            'Mins: %{customdata[0]:,}<br>'
            'Own: %{customdata[1]:.1f}%<extra></extra>'
        ),
        customdata=top_25[['minutes', 'ownership']].values
    ))
    bar_fig.update_layout(
        template='plotly_white', height=420, xaxis_tickangle=-45,
        yaxis_title=metric_label, showlegend=False,
        yaxis=dict(range=[0, top_25[metric].max() * 1.18]) if len(top_25) > 0 else {},
        font=dict(family='Arial, sans-serif')
    )

    # Table
    table_cols = ['web_name', 'team_name', 'position', 'price', 'minutes',
                  'xg_per_90', 'xa_per_90', 'xgi_per_90',
                  'goals_per_90', 'assists_per_90', 'bps_per_90', 'ownership']
    table_data = prepare_table_data(
        filtered.dropna(subset=[metric]).nlargest(60, metric),
        [c for c in table_cols if c in filtered.columns]
    )
    return scatter_fig, bar_fig, table_data
...
if __name__ == '__main__':
    ...
    print("  Squad Builder, BGW/DGW Planner, Underlying Stats/90")
    print("=" * 60)
    print("\n  Starting server...")
