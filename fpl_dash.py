...
def process_player_data(data):
    ...
    df['defcon_per_90'] = (df['defcon'] / df['minutes_safe']) * 90
...
best_value = df_active[df_active['minutes'] > 450].nlargest(1, 'points_per_million').iloc[0] if len(
    df_active[df_active['minutes'] > 450]) > 0 else None
...
home_value_cols = ['web_name', 'team_name', 'position', 'price', 'total_points', 'points_per_million', 'form',
                   'ownership']
home_table_data = prepare_table_data(df_active[df_active['minutes'] > 450].nlargest(10, 'points_per_million'),
                                     home_value_cols)
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.Div([
                html.Span("FPL", style={'backgroundColor': COLORS['secondary'], 'color': COLORS['primary'],
            ...
            dcc.Tab(label='Home', value='home', children=[
                html.Div([
                    ...
                    html.Div([
                        ...
                        html.Div([build_stat_card("Current Gameweek", current_gw['name'].replace('Gameweek ',
                                                                                                 'GW') if current_gw else "N/A",
                                                  f"Average: {avg_gw_score} pts")],
                                 style={'flex': '1', 'minWidth': '200px', 'padding': '0 10px'}),
                        html.Div([build_stat_card("Highest GW Score", f"{highest_gw_score}", "This gameweek",
                                                  color=COLORS['success'])],
                                 style={'flex': '1', 'minWidth': '200px', 'padding': '0 10px'}),
                        html.Div([build_stat_card("Next Deadline",
                                                  next_gw['name'].replace('Gameweek ', 'GW') if next_gw else "N/A",
                                                  datetime.fromisoformat(
                                                      next_gw['deadline_time'].replace('Z', '+00:00')).strftime(
                                                      '%d %b, %H:%M') if next_gw else "")],
                                 style={'flex': '1', 'minWidth': '200px', 'padding': '0 10px'}),
                    ...
                    html.Div([
                        build_player_spotlight(top_scorer, "Top Scorer", "Total Points",
                                               f"{int(top_scorer['total_points'])}" if top_scorer is not None else "N/A"),
                        build_player_spotlight(most_selected, "Most Selected", "Ownership",
                                               f"{most_selected['ownership']:.1f}%" if most_selected is not None else "N/A"),
                        build_player_spotlight(best_value, "Best Value", "Points/£m",
                                               f"{best_value['points_per_million']:.2f}" if best_value is not None else "N/A"),
                        build_player_spotlight(top_form, "In Form", "Form Rating",
                                               f"{top_form['form']:.1f}" if top_form is not None else "N/A"),
                    ...
                    html.Div([dcc.Graph(figure=build_position_breakdown_chart(), config={'displayModeBar': False})],
                             style=CARD_STYLE),
            ...
            dcc.Tab(label='Defcon Bonus', value='defcon-bonus', children=[
                html.Div([
                    html.Div([
                        ...
                        html.Div(
                            [html.Span("🎯 Target: 10 defcon per 90", style={'backgroundColor': COLORS['secondary'],
                                                                              'color': COLORS['primary'],
                                                                              'padding': '8px 16px',
                                                                              'borderRadius': '20px',
                                                                              'fontWeight': '600'})])
                        ...
                        html.Div([
                            ...
                            html.Div([
                                html.Label("Max Price",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Slider(id='bonus-price', min=4, max=15, step=0.5, value=15,
                                           marks={i: f'£{i}' for i in [4, 6, 8, 10, 12, 15]})
                    ...
                    html.Div([
                        ...
                        html.P("Red line = 10-point bonus threshold. Players above consistently earn defcon bonuses.",
                    ...
                    html.Div([
                        html.H3("Distance from Bonus Threshold",
                                style={'color': COLORS['primary'], 'marginBottom': '8px'}),
                        html.P("How far above or below the 10-point threshold each player averages.",
                               style={'color': COLORS['text_light']}),
                    ...
                    html.Div([
                        html.Div([
                            ...
                            html.Div([
                                html.Label("Max Price",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Slider(id='defcon-price', min=4, max=15, step=0.5, value=15,
                                           marks={i: f'£{i}' for i in [4, 6, 8, 10, 12, 15]})
                            ], style={'flex': '2', 'minWidth': '200px', 'padding': '0 10px'}),
                            html.Div([
                                html.Label("Min Minutes",
                    ...
                    html.Div([
                        html.Div([
                            ...
                            html.Div([
                                html.Label("Max Price",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Slider(id='xg-price', min=4, max=15, step=0.5, value=15,
                                           marks={i: f'£{i}' for i in [4, 6, 8, 10, 12, 15]})
                            ], style={'flex': '2', 'minWidth': '200px', 'padding': '0 10px'}),
                            html.Div([
                                html.Label("Min Minutes",
                    ...
                    html.Div([
                        html.Div([
                            ...
                            html.Div([
                                html.Label("Max Price",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Slider(id='value-price', min=4, max=15, step=0.5, value=15,
                                           marks={i: f'£{i}' for i in [4, 6, 8, 10, 12, 15]})
                            ], style={'flex': '2', 'minWidth': '200px', 'padding': '0 10px'}),
                            html.Div([
                                html.Label("Min Minutes",
                    ...
                    html.Div([
                        html.Div([
                            ...
                            html.Div([
                                html.Label("Max Price",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Slider(id='form-price', min=4, max=15, step=0.5, value=15,
                                           marks={i: f'£{i}' for i in [4, 6, 8, 10, 12, 15]})
                            ], style={'flex': '2', 'minWidth': '200px', 'padding': '0 10px'}),
                            html.Div([
                                html.Label("Min Minutes",
                    ...
                    html.Div([
                        html.Div([
                            ...
                            html.Div([
                                html.Label("Max Price",
                                           style={'fontWeight': '600', 'marginBottom': '6px', 'display': 'block'}),
                                dcc.Slider(id='cs-price', min=4, max=15, step=0.5, value=15,
                                           marks={i: f'£{i}' for i in [4, 6, 8, 10, 12, 15]})
                            ], style={'flex': '2', 'minWidth': '200px', 'padding': '0 10px'}),
                            html.Div([
                                html.Label("Min Minutes",
